...
export async function provisionaSchemaSpazio(codiceSpazio: string): Promise<string> {
  ...
  await eseguiDdlTenant(
    sql`CREATE UNIQUE INDEX IF NOT EXISTS admin_workspace_username_key ON ${s}.admin_workspace (username) WHERE username IS NOT NULL`
  );
...
export async function assicuraTabelleUtenti(nomeSchema: string): Promise<void> {
  ...
  await eseguiDdlTenant(
    sql`CREATE UNIQUE INDEX IF NOT EXISTS utenti_spazio_username_key ON ${s}.utenti_spazio (username) WHERE username IS NOT NULL`
  );
