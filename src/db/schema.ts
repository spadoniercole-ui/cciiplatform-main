...
export function getTabelleTenant(nomeSchema: string) {
  ...
  return {
    ...
    admin_workspace: tableBuilder('admin_workspace', {
      ...
      // Identità di login: "nome.cognome" (+2 cifre per omonimia). Chiave
      // reale dell'accesso dalla 0.109 — vedi src/lib/generaUsername.ts.
      // Nullable a livello di tipo per gli schemi già esistenti prima della
      // migrazione (il backfill la valorizza); UNIQUE a livello di DDL.
      username: text('username'),
      // L'email non è più identità di login: torna a essere un dato di
      // contatto, modificabile e non più vincolo di unicità globale.
    ...
    utenti_spazio: tableBuilder('utenti_spazio', {
      ...
      nome: text('nome').notNull(),
      cognome: text('cognome').notNull(),
      // Identità di login (come admin_workspace): "nome.cognome" (+cifre).
      username: text('username'),
      email: text('email').notNull().unique(),
