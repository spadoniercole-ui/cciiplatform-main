...
export default function SpaziPage() {
  ...
  const [usernameCredenziali, setUsernameCredenziali] = useState<string | null>(null);
  ...
  const handleScaricaCredenziali = () => {
    if (!ultimoCodiceCreato || !passwordGenerata || !usernameCredenziali) return;
...
Login (nome utente): ${usernameCredenziali}
Password temporanea: ${passwordGenerata}
Email di contatto: ${emailCredenziali || '—'}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (risultato.passwordTemporanea) {
        ...
        setUsernameCredenziali(risultato.username || null);
      }
      ...
      >
        ...
        {passwordGenerata && (
          <div className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-1">
            <div className="font-bold uppercase tracking-wider text-[10px]">
              Credenziali dell&apos;Admin di Spazio — mostrate una sola volta
            </div>
            {usernameCredenziali && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold uppercase text-amber-700 w-24 shrink-0">
                  Nome utente
                </span>
                ...
                <button
                  type="button"
                  onClick={() => navigator.clipboard.writeText(usernameCredenziali)}
            ...
            <p className="text-[10px]">
              L&apos;Admin accede con il <strong>nome utente</strong> (non con l&apos;email).
              Comunica nome utente e password per un canale sicuro: dovrà cambiare la password al
              primo accesso.
            </p>
            <button
              type="button"
        ...
        <div className="border-t border-slate-100 pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">
                Email (contatto, facoltativa)
              </label>
              <input
                type="email"
                value={adminEmail}
                onChange={(e) => setAdminEmail(e.target.value)}
                placeholder="solo contatto — il login è il nome utente"
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-blue-500 text-slate-900"
              />
              <p className="text-[10px] text-slate-400 mt-1">
                Il nome utente di accesso (nome.cognome) viene generato automaticamente e mostrato
                dopo la creazione.
              </p>
            </div>
            <div>
