...
async function creaSessione(
  ...
  await pool.query(
    'INSERT INTO sessioni (token, ruolo, workspace_id, email, expires_at) VALUES ($1, $2, $3, $4, $5)',
    [token, ruolo, workspaceId, email || null, scadenza]
  );
...
/**
 * Cerca un'email tra gli admin_workspace di TUTTI gli schemi tenant
 * esistenti (per spazi creati prima che admin_spazio_index esistesse, o
 * per qualunque motivo l'indice non fosse aggiornato). Se la trova,
 * ripara l'indice al volo: la prossima volta il login la trova subito,
 * senza dover ripetere la scansione né richiedere SQL manuale.
 */
async function cercaERiparaIndiceAdmin(email: string): Promise<SchemaAdminTrovato | null> {
  try {
    ...
    for (const riga of schemiRisultato.rows) {
      ...
      try {
        ...
        continue; // schema senza tabella admin_workspace, o altro problema: salta e prova il prossimo
      }
      ...
      if (trovato.rows.length > 0) {
        ...
        await pool.query(
          ...
          [email, nomeSchema, spazioId, codiceSpazio]
        );
 ...
 * utenti_spazio (Operativo/Consultatore) invece di admin_workspace.
 */
async function cercaERiparaIndiceUtente(email: string): Promise<SchemaAdminTrovato | null> {
  try {
    const schemiRisultato = await pool.query(
      `SELECT schema_name FROM information_schema.schemata WHERE schema_name LIKE 'tenant\\_%' ESCAPE '\\'`
    );
    ...
    for (const riga of schemiRisultato.rows) {
      ...
      try {
        trovato = await pool.query(
          `SELECT email FROM "${nomeSchema}".utenti_spazio WHERE email = $1`,
          [email]
        );
      } catch {
        continue;
      ...
      if (trovato.rows.length > 0) {
        ...
        await pool.query(
          `INSERT INTO utente_spazio_index (email, nome_schema, spazio_id, codice_spazio)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (email) DO NOTHING`,
          [email, nomeSchema, spazioId, codiceSpazio]
        );
...
export async function eseguiAutenticazione(utenteInput: any, passwordInput: any) {
  try {
    ...
    try {
      const { assicuraIndiceAdminSpazio, assicuraIndiceUtenteSpazio } =
        await import('@/db/ensureTables');
      await assicuraIndiceAdminSpazio();
      await assicuraIndiceUtenteSpazio();

      const emailNormalizzata = utente.toLowerCase();

      const indiceAdminRisultato = await pool.query(
        'SELECT nome_schema, spazio_id, codice_spazio FROM admin_spazio_index WHERE email = $1',
        [emailNormalizzata]
      );
      ...
      const schemaAdmin: SchemaAdminTrovato | null =
        indiceAdminRisultato.rows.length > 0
          ...
          : await cercaERiparaIndiceAdmin(emailNormalizzata);
      ...
      if (schemaAdmin) {
        ...
        const utenteDb = await db
          ...
          .where(eq(tabelleSpazio.admin_workspace.email, emailNormalizzata))
          .limit(1);
        ...
        if (utenteDb.length > 0) {
          ...
          await creaSessione('USER', schemaAdmin.spazioId, emailNormalizzata);
          return {
            success: true,
            role: 'USER',
            goToChoice: false,
      ...
      const indiceUtenteRisultato = await pool.query(
        'SELECT nome_schema, spazio_id, codice_spazio FROM utente_spazio_index WHERE email = $1',
        [emailNormalizzata]
      );
      ...
      const schemaUtente: SchemaAdminTrovato | null =
        indiceUtenteRisultato.rows.length > 0
          ...
          : await cercaERiparaIndiceUtente(emailNormalizzata);
      ...
      const utenteDb = await db
        ...
        .where(eq(tabelleSpazio.utenti_spazio.email, emailNormalizzata))
        .limit(1);
      ...
      await creaSessione('USER', schemaUtente.spazioId, emailNormalizzata);
      return {
        success: true,
        role: 'USER',
        goToChoice: false,
