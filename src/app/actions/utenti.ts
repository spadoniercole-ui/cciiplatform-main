...
export async function creaUtenteSpazioAction(
  ...
  try {
    ...
    const inserito = await db
      ...
      .values({ nome, cognome, email, tipologia: dati.tipologia, passwordHash, passwordTemporanea })
    ...
    // Indicizzazione email -> schema: senza questo, il login non saprebbe
    // in quale schema isolato cercare le credenziali di questa email
    // (stesso principio già usato per admin_spazio_index).
    ...
    if (spazioRisultato.rows.length > 0) {
      await pool.query(
        ...
        [email, nomeSchema, spazioRisultato.rows[0].id, spazioRisultato.rows[0].codice]
      );
    }
