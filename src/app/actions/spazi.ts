...
): Promise<ActionResult & { codice?: string; passwordTemporanea?: string }> {
  try {
    ...
    if (!admin?.email?.trim() || !admin.email.includes('@')) {
      return { success: false, error: "Email dell'Admin di Spazio non valida." };
    }
    ...
    // Verifica PRIMA di creare qualunque cosa — l'indice globale
    // (admin_spazio_index) ha email come chiave primaria unica su
    // tutta la piattaforma: un secondo Admin con la stessa email di un
    // altro spazio sovrascriveva silenziosamente l'indice, facendo
    // "sparire" il primo admin (il suo account restava intatto nel
    // proprio schema, ma il login non lo trovava più — puntava al
    // nuovo spazio). Bloccato qui, prima della transazione, non dopo.
    {
      await assicuraIndiceAdminSpazio();
      const emailNormalizzata = admin.email.trim().toLowerCase();
      const giaInUso = await pool.query(
        `SELECT s.descrizione, s.codice FROM admin_spazio_index i
         JOIN spazi s ON s.id = i.spazio_id
         WHERE i.email = $1`,
        [emailNormalizzata]
      );
      if (giaInUso.rows.length > 0) {
        const spazioEsistente = giaInUso.rows[0];
        return {
          success: false,
          error: `Questa email è già l'Admin dello spazio "${spazioEsistente.descrizione}" (${spazioEsistente.codice}) — usa un'email diversa per questo nuovo spazio.`,
        };
      }
    }
    ...
    try {
      ...
      try {
        ...
        // Indicizzazione email -> schema: senza questo, il login non
        // saprebbe in quale schema isolato cercare le credenziali di
        // questa email (vedi eseguiAutenticazione in actions/auth.ts).
        await assicuraIndiceAdminSpazio();
        await pool.query(
          ...
          [admin.email.trim().toLowerCase(), nomeSchema, spazioId, codice]
        );

        revalidatePath('/superadmin/Spazi');
        return { success: true, codice, passwordTemporanea };
...
export async function riparaIndiceAdminAction(
  ...
  try {
    ...
    const presente = await pool.query(
      `SELECT email FROM "${nomeSchemaScelto}".admin_workspace WHERE email = $1`,
      [emailNormalizzata]
    );
    ...
    await pool.query(
      `INSERT INTO admin_spazio_index (email, nome_schema, spazio_id, codice_spazio)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email) DO UPDATE SET nome_schema = EXCLUDED.nome_schema, spazio_id = EXCLUDED.spazio_id, codice_spazio = EXCLUDED.codice_spazio`,
      [emailNormalizzata, nomeSchemaScelto, spazioRis.rows[0].id, spazioRis.rows[0].codice]
    );
...
export async function ottieniContestoAccessoSpazio(
  ...
  try {
    ...
    const risultato = await pool.query(
      `SELECT s.ruolo, s.email, sp.id AS spazio_id, sp.codice, sp.descrizione, sp.nome_schema,
    ...
    if (riga.email) {
      ...
      const adminRighe = await db
        ...
        .from(tabelle.admin_workspace)
        .where(eq(tabelle.admin_workspace.email, riga.email))
        .limit(1);
    ...
    if (riga.email) {
      ...
      const utenteRighe = await db
        .select()
        .from(tabelle.utenti_spazio)
        .where(eq(tabelle.utenti_spazio.email, riga.email))
        .limit(1);
