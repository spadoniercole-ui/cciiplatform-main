...
async function creaSpazioPortable(cfg: ConfigSpazio): Promise<SpazioCreato> {
  ...
  // 3) Admin di Spazio + indice username -> schema per il login. Lo
  //    username (nome.cognome + eventuali cifre) è la chiave di accesso.
  const username = await generaUsernameUnivoco(cfg.admin.nome, cfg.admin.cognome, (u) =>
  ...
  await creaAdminSpazio(nomeSchema, {
    ...
    // definitiva. Il campo deve restare NULL: la stringa vuota veniva letta
    // come "password temporanea presente" e il layout dello spazio
  ...
  await pool.query(
    ...
    [username, email, nomeSchema, spazioId, codice]
  );

  console.info(
    `[portable] Spazio "${cfg.descrizione}" (${codice}, ${cfg.tipoSpazio}) creato. Login (username): ${username}`
  );
...
export async function bootstrapPortable(): Promise<void> {
  ...
  // La chiave di login è ora lo USERNAME (nome.cognome), non l'email:
  // le due email possono anche coincidere senza problemi. Gli username
  // dei due admin di default (Admin Redigente / Admin Ricevente) sono
  // comunque distinti — "admin.redigente" e "admin.ricevente".
