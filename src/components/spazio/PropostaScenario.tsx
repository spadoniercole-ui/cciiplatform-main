...
export function PropostaScenario({
  ...
  const carica = async () => {
    ...
    try {
      ...
      if (risultatoRighe.success) {
        setRighe(risultatoRighe.righe);
        setVersioneRighe((v) => v + 1);
            ...
            <>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  {righe.length > 0
                    ? 'Esporta modello (con righe attuali)'
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          percentualeOfferta: parseNumeroItaliano(e.target.value),
                        })
                      }
                      className="w-full p-2 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900"
                    />
                  ...
                  <div className="w-32">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          numeroRate: e.target.value ? Number(e.target.value) : null,
                        })
                      }
                      className="w-full p-2 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900"
                    />
          ...
          {tipoProposta === 'RICEVUTA' && (
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              {(() => {
                ...
                const totaleDebitiEnte = debitiEnte.reduce(
                  (acc, r) => acc + saldoRigaDebitoEnte(r),
                  0
                );
                    ...
                    <div className="flex items-start gap-2 text-xs text-slate-500">
                      ...
                      <p>
                        {giaAnalizzatoSenzaValore ? (
                          <>
                            L&apos;analisi è stata fatta, ma non ha trovato un importo chiaro per
                            questo ente: <span className="italic">{rigaEstratta!.motivazione}</span>{' '}
                            — aggiungi gli alias di questo ente in Parametri di Spazio se il
                            documento lo chiama con un nome diverso, poi rianalizza qui sotto.
                          </>
                        ) : (
                          <>
                            Carica e analizza la proposta di cram down qui sotto per vedere il
                            confronto — l&apos;importo offerto lo estrae l&apos;AI dal documento.
                          </>
                        )}
                      </p>
                    </div>
                ...
                return (
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    >
                      <span className="text-[10px] text-slate-500 uppercase font-bold block">
                        Riscontro con i parametri dell’ente
                      </span>
                      <span className="text-sm font-bold text-slate-900">
                        {rigaEstratta.ricevibile
                          ? 'Coerente con i parametri configurati'
                          : 'Non coerente con i parametri configurati'}
                      </span>
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        {rigaEstratta.motivazione}
                      </p>
