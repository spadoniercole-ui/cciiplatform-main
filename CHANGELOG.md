...
Sette versioni su questo errore. La svolta e' arrivata dal log di Vercel
fornito da Ercole: **500 in 514 ms, nessuna richiesta in uscita**. Non un
timeout, non memoria — un fallimento immediato, prima di toccare il database.
Con quel dato ho potuto RIPRODURRE il guasto in locale (backup gonfiato a
4,29 MB nel sandbox portable), e il messaggio vero e' comparso:

    Maximum array nesting exceeded. Large nested arrays can be dangerous.

E' il serializzatore delle Server Action, che non regge un argomento stringa
da 1,5 MB. Non c'entravano ne' il limite dei 4,5 MB, ne' `bodySizeLimit`, ne'
la durata: era la dimensione del singolo frammento.

**Frammenti portati da 1,5 MB a 250 KB.** Valore determinato per prova, non
per stima: a 250 KB il caricamento funziona. Un file da 4,3 MB viaggia in 19
parti.
...
multi-istruzione si riconosce ora PRIMA di eseguire, con uno scanner che
ignora i punti e virgola dentro le stringhe (una ragione sociale come
"Alfa; Beta") e dentro i commenti. **7 test**, uno dei quali ha bocciato la
prima stesura della funzione.

**Collaudo end-to-end nel sandbox portable**, ciclo intero: backup generato,
gonfiato a 4,29 MB, caricato in 19 parti, ricomposto, validato, eseguito in
prova su transazione annullata ("il database reale non e' stato toccato"),
poi ripristinato davvero — 64 tabelle — con il backup di sicurezza scaricato
automaticamente. Nessun errore nei log del server.

Verificato: type-check, lint, **160 test**, build completa.

**Nota di metodo.** Le sei versioni precedenti hanno corretto difetti reali —
uno era un buco di sicurezza serio — ma nessuno era la causa del blocco,
perche' ho cercato di dedurla invece di riprodurla. Il log e' arrivato, e in
un'ora si e' chiuso tutto. La lezione: davanti a un errore opaco, riprodurlo
viene prima di correggerlo.
